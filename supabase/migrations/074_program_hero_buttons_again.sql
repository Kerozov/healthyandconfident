-- 074: the big button at the top of each programme page gets its own row
-- (second attempt).
--
...
--
-- 072 did the same and 073 undid it: the site was rebuilt before the rows
-- existed and the button lost its link. The page now falls back to the base
-- row while `_hero` is missing (lib/site/cta-placements.ts), so the order of
-- deploy and migration no longer matters.
