...
insert into public.site_cta_placements (key, label_bg, label_en) values
  ...
  ('outcomes_cta', 'Секция „Резултати“ — бутон „Виж програмите“', 'Outcomes section — “See the programs” button'),
  ('leadmagnet_cta', 'Безплатно 2-дневно меню — popup след запис на имейл', 'Free 2-day menu — popup after email signup'),
  ('contact_cta', 'Контакти — връзка към програмите', 'Contact — link to programs'),
  ...
  ('nav_cta', 'Горно меню — „Виж програмите“', 'Top navigation — see the programs CTA'),
...
create table if not exists public.email_footer_config (
  ...
  header_image_full_width boolean not null default false,
  header_bg_color     text not null default '#2D7A47',
  copyright_enabled   boolean not null default true,
  signature_links     jsonb not null default '[]',
  updated_at          timestamptz not null default now()
);
...
-- 046: reporting indexes, order totals, Meta Pixel
...
-- Site visits — first-party traffic (050)
