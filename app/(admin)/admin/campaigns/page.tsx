...
import { CampaignTracking } from "@/components/admin/campaign-tracking";
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink-soft">
        Send segmented email &amp; SMS to your subscribers and track opens.
      </p>
      ...
      <div className="mt-8">
        <Card title="Email campaigns">
          {emailCampaigns.length === 0 ? (
            <p className="text-sm text-ink-soft">No email campaigns yet.</p>
          ) : (
                ...
                <tbody>
                  {emailCampaigns.map((c) => (
                    <tr key={c.id} className="border-b border-ink/5 align-top last:border-0">
                      <td className="py-3 pr-4 font-medium">{c.subject}</td>
                      ...
                      <td className="py-3 pr-4">
                        <CampaignTracking jobId={c.worker_job_id} />
                      </td>
                    </tr>
                  ))}
                </tbody>
